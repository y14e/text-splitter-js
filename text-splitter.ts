  ...
  private split(by: 'word' | 'char', node = this.fragment as unknown as ChildNode): void {
    ...
    [...node.childNodes].forEach(node => {
      ...
      if (node.nodeType === Node.TEXT_NODE) {
        ...
        const segments = [...new Intl.Segmenter(((parent.nodeType === Node.ELEMENT_NODE ? parent : this.rootElement) as HTMLElement).closest('[lang]')?.getAttribute('lang') || document.documentElement.getAttribute('lang') || 'en', by === 'word' && this.settings.wordSegmenter ? { granularity: 'word' } : {}).segment(text.replace(/[\r\n\t]/g, '').replace(/\s{2,}/g, ' '))];
